...
fn current_task_id() -> Option<&'static str> {
    env::var("PRAXIS_KANBAN_TASK").ok().map(|s| Box::leak(s.into_boxed_str()) as &'static str)
}
...
pub fn handle_kanban_create(
    ...
    let body = params.get("body").map(|v| v.as_str());
    ...
    let task = store.create_task(title, body, priority, assignee, parent_ids, labels)?;
