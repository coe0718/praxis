...
Terminal UI — only compiled when the `tui` feature is enabled.
...
### Testing
```bash
cargo test --package praxis-tui --lib
```


## Usage

As a library module, tui is typically used internally by the Praxis framework. 
Consult the source code and module documentation for specific usage patterns.

### Importing
In your Rust code:
```rust
use praxis::tui::*;  // or specific components as needed
```
...
*Documentation auto-generated for Praxis module `tui`*
