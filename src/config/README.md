...
Config module for Praxis AI agent framework
...
### Testing
```bash
cargo test --package praxis-config --lib
```


## Usage

As a library module, config is typically used internally by the Praxis framework. 
Consult the source code and module documentation for specific usage patterns.

### Importing
In your Rust code:
```rust
use praxis::config::*;  // or specific components as needed
```
...
*Documentation auto-generated for Praxis module `config`*
