...
pub fn execute_request(
    ...
    let vault = Vault::load(&paths.vault_file).unwrap_or_else(|e| {
    ...
    let mut result = match manifest.name.as_str() {
        ...
        "browser" => {
            let payload = parse_payload(request.payload_json.as_deref())?;
            let params = serde_json::to_value(&payload.params).unwrap_or(serde_json::Value::Null);
            match crate::tools::browser::execute_browser_tool(&params) {
                Ok(output) => Ok(ToolExecutionResult { summary: output }),
                Err(e) => Ok(ToolExecutionResult {
                    summary: format!("browser error: {e}"),
                }),
            }
        }
        "spotify" => {
            let payload = parse_payload(request.payload_json.as_deref())?;
            let params = serde_json::to_value(&payload.params).unwrap_or(serde_json::Value::Null);
            match crate::spotify::execute_spotify_tool(&params) {
                Ok(output) => Ok(ToolExecutionResult { summary: output }),
                Err(e) => Ok(ToolExecutionResult {
                    summary: format!("spotify error: {e}"),
                }),
            }
        }
        "meet" => {
            let payload = parse_payload(request.payload_json.as_deref())?;
            let params = serde_json::to_value(&payload.params).unwrap_or(serde_json::Value::Null);
            match crate::meet::execute_meet_tool(&params) {
                Ok(output) => Ok(ToolExecutionResult { summary: output }),
                Err(e) => Ok(ToolExecutionResult {
    ...
    if redact_secrets && let Ok(ref mut r) = result {
